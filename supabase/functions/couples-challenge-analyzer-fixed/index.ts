...
Provide a comprehensive analysis that helps them understand their relationship dynamics and grow together.`;
...
serve(async (req) => {
  ...
  try {
    console.log('Couples challenge analyzer called');
    ...
    console.log('Challenge found, processing messages');
    ...
    if (userMessages.length !== questions.length || partnerMessages.length !== questions.length) {
      throw new Error('Incomplete responses - not all questions answered');
    }

    console.log('Analyzing responses with Z.AI...');
    ...
    console.error('Couples challenge analyzer error:', error);
    return new Response(
      JSON.stringify({
        success: false,
