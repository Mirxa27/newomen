...
serve(async (req) => {
  ...
  try {
    ...
    console.error("provider-discovery-simple error", error);
...
async function handleCreateProvider(supabase: ReturnType<typeof createClient>, payload?: ProviderPayload) {
  ...
  // Store API key in a simple way (without encryption for now)
  try {
    ...
    console.warn("Could not store encrypted API key, continuing without encryption:", error);
    // Continue without encryption - this is a fallback
  }
...
async function handleSyncProvider(
  ...
  if (!apiKey) {
    try {
      ...
      console.warn("Could not retrieve encrypted API key:", error);
      // Continue without API key - this might limit functionality
    }
  }
